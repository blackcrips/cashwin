...
// let buttonNextSubmit = document.getElementById("next-submit");
...
buttonNext.onclick = () => {
  ...
  if (countDataValidationLength == 0) {
    ...
    if (confirm("Continue to save information?") == true) {
      ...
      buttonNextSubmit.setAttribute("name", "submit-edit");
...
buttonBack.onclick = () => {
  if (confirm("Save actions before closing the browser!") == true) {
    let container = document.getElementById("container-button");
    let buttonNextSubmit = document.createElement("button");
    buttonNextSubmit.setAttribute("type", "Submit");
    buttonNextSubmit.setAttribute("name", "submit-edit");
